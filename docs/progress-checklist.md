...
- [x] FA-07: Segmentierung (`lib/ai/segment.ts`, Split an `/\n\s*\n/`) + `retranslateSchema` (ohne `detectedSourceLanguage` — zum Zeitpunkt der Neuübersetzung bereits bekannt)
- [x] FA-07: `app/api/retranslate/route.ts` (`streamText` + `Output.object`, Prompt mit getrennten `Segment:`/`Comment:`-Abschnitten und explizitem "übersetze den Kommentar nie mit", Fallback-Instruktion für leeren Kommentar). Keine Persistenz — Neuübersetzungen sind Session-State (siehe FA-07-Wortlaut)
