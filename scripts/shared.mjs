...
// Node refuses to spawn .cmd shims directly since CVE-2024-27980, so on Windows the
// Supabase CLI has to go through a shell — see the quoting in run().
...
export const run = (command, args, { capture = false, check = true } = {}) => {
  ...
  const result = spawnSync(viaShell ? `"${command}"` : command, args, {
    ...
    // Quoted because the repository path may contain spaces.
    shell: viaShell,
    encoding: 'utf8',
  });
